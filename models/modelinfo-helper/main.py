#!/usr/bin/env python3
# SPDX-FileCopyrightText: 2025 Daniel Morin <daniel.morin@collabora.com>
# SPDX-FileCopyrightText: 2026 Olivier Crête <olivier.crete@collabora.com>
...
def get_tensor_info(tensor, direction, group_id=None, prompt_mode=False, model=None):
    ...
    if prompt_mode:
        ...
        info["id"] = prompt_for_value(
            f"Enter tensor ID", default=default_id, newline=True
        )

        if prompt_yes_no("Specify dims-order (row-major/col-major)?", default=False):
            while True:
                dims_order = (
                    input("  Enter dims-order [row-major/col-major]: ").strip().lower()
                )
        ...
        if direction == "input":
            ...
            if model and hasattr(model, "metadata_props"):
                for prop in model.metadata_props:
                    if prop.key == "Image.NominalPixelRange":
                        ...
                        if ranges_from_onnx:
                            print(
                                f"  Found Image.NominalPixelRange in model: {prop.value}"
                            )
                        break
            ...
            if ranges_from_onnx:
                # Confirm with user
                if prompt_yes_no(
                    f"  Use ranges {ranges_from_onnx} from model metadata?",
                    default=True,
                ):
                    ...
                    ranges = prompt_for_value(
                        "  Enter ranges (min,max)", default="0.0,255.0"
                    )
                    info["ranges"] = ranges
            else:
                ...
                for prop in model.metadata_props:
                    if prop.key == "Image.NominalPixelRange":
                        ...
                        if ranges_from_onnx:
                            info["ranges"] = ranges_from_onnx
                            print(
                                f"  Input tensor '{tensor.name}': Using ranges from model metadata: {prop.value}"
                            )
                        break
...
def load_tflite_model(model_path):
    ...
    if not TFLITE_AVAILABLE:
        print(
            "Error: tensorflow package is required for TFLite support. Install with: pip install tensorflow",
            file=sys.stderr,
        )
        sys.exit(1)
...
def generate_modelinfo(model_path, output_path=None, prompt_mode=False):
    ...
    if model_type == "onnx":
        if not ONNX_AVAILABLE:
            print(
                "Error: onnx package is required for ONNX support. Install with: pip install onnx",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        interpreter, input_details, output_details, model_name = load_tflite_model(
            model_path
        )
    ...
    if prompt_mode:
        ...
        print(
            "https://github.com/collabora/tensor-id-registry/blob/main/tensor-id-register.md"
        )
    else:
        print(
    ...
    for tensor_detail in model_inputs:
        ...
        tensor_info = get_tensor_info(
            tensor, "input", prompt_mode=prompt_mode, model=model if is_onnx else None
        )
    ...
    for tensor_detail in model_outputs:
        ...
        tensor_info = get_tensor_info(
            tensor, "output", group_id=group_id, prompt_mode=prompt_mode
        )
        all_tensors.append(tensor_info)
    ...
    write_modelinfo(
        all_tensors, output_path, group_id=group_id, prompt_mode=prompt_mode
    )
    print(f"\nSuccessfully generated modelinfo file: {output_path}")


def write_modelinfo(
    tensors, output_path, version=None, group_id=None, prompt_mode=False
):
    ...
    with open(output_path, "w") as f:
        ...
        if not prompt_mode:
            ...
            f.write(
                "# Tensor ID Registry: https://github.com/collabora/tensor-id-registry/blob/main/tensor-id-register.md\n"
            )
            f.write("#\n")
...
def parse_modelinfo(input_path):
    ...
    for section in config.sections():
        ...
        if (
            global_group_id
            and tensor.get("dir") == "output"
            and "group-id" not in tensor
        ):
...
def upgrade_modelinfo(input_path, output_path=None):
    ...
    if old_major != new_major:
        print(
            f"Error: Cannot upgrade from v{old_major}.x to v{new_major}.x",
            file=sys.stderr,
        )
        print(
            f"Cross-major version upgrades are not supported at this time.",
            file=sys.stderr,
        )
        sys.exit(1)
